...
- **Radios anidados**: nuevos tokens `--radius-inset-1`/`--radius-inset-2` (`radio-lg − padding`, ver `tokens.css`) para que el ítem de un menú calce con la esquina del contenedor. Aplicados en `Select` (Viewport `p-1`) y `DropdownMenu` (Content `p-2`), que antes usaban `rounded-sm` fijo sin relación con el `rounded-lg` del contenedor.
