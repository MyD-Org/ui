...
### Changelog 0.32.0 (aditivo salvo un default visual; sin cambios de API)
- **Radios de menú**: nuevos tokens `--radius-menu-1`/`--radius-menu-2` (`radio-sm + padding`, ver `tokens.css`). El ítem de `Select` (Viewport `p-1`) y de `DropdownMenu` (Content `p-2`) sigue en `rounded-sm` y el contenedor se deriva de él, así las dos curvas son concéntricas. Antes el contenedor era `rounded-lg` sin relación con el ítem.
- **Escala de radios extendida**: `--radius-md`, `--radius-xl` y `--radius-2xl` como alias de `--radius-sm`, `--radius` y `--radius-lg` (8/12/18px en la piel base), mapeados en `tailwind.css`. Antes `rounded-md`/`-xl`/`-2xl` no estaban mapeados y caían a los valores de fábrica de Tailwind (6/12/16px): un consumidor que use esas utilidades ahora sigue la piel de su app en vez de quedar con una esquina ajena a la escala. `rounded-3xl`/`-4xl` siguen sin token (la guarda de `tokens.test.ts` los sigue atrapando).
- **`Button`**: `size="icon"`/`"icon-lg"` suman `shrink-0` — sin eso, dentro de un flex que se queda corto de espacio el botón se achicaba de ancho y dejaba de ser cuadrado.
- **`ProductCard`** (cambio visual del default): `imageWrap` pasa de `bg-elevated/50` a `bg-surface` con un borde sutil (`border-border/60`, abajo en `layout="grid"`, a la derecha en `layout="list"`). `elevated` es un rol que cada piel tiñe libremente (en el theme azul del Shop es celeste) y quedaba como un tinte de color detrás de fotos con fondo blanco/transparente.
- **`Dialog`**: entrada/salida animada con Radix Presence (`data-[state]`, `animation` CSS, ease-out `cubic-bezier(0.16,1,0.3,1)`, salida más corta que la entrada) — overlay con fade, contenido `center` con fade + scale 0.96→1, `sheet` con slide vertical. `motion-reduce:animate-none` la saca sin sacar el fade/aparición.
- **`Toast`**: mismo criterio de entrada/salida (slide + fade); como no usa Radix, `ToastProvider.dismiss` ahora espera la animación de salida (160 ms) antes de sacar el toast de la lista, salvo con `prefers-reduced-motion`.
...
- **`PromoBanner`**: `renderImage` (`fit: 'cover'`, sin `priority`) e `imageSizes` (default `'100vw'`). La foto por defecto suma `loading="lazy"` + `decoding="async"`.
- **`RoomTiles`**: `renderImage`, `renderLink` (recibe `data-size="big"` en la destacada del mosaico) e `imageSizes`, con default por variante: mosaic `(min-width: 1024px) 50vw, 100vw`, grid `(min-width: 1024px) 25vw, 50vw`, stack `100vw`. Las fotos por defecto suman `loading="lazy"` + `decoding="async"`.
- **`Marquee`**: `renderImage` para los logos (`fit: 'logo'`, `sizes: '150px'`, `alt: ''`, `data-logo`). Sin renderer el `<img>` del logo no cambia.
- **`RenderLinkProps`** acepta atributos `data-*` opcionales.

### Changelog 0.29.0 (aditivo: los defaults no cambian)
- **`SiteFooter`**: `SiteFooterLink.external` abre el enlace en una pestaña nueva (`target="_blank"` + `rel="noopener noreferrer"`) y **no** pasa por `renderLink` (un `next/link` a otro sitio no aporta nada). Nueva prop `barExtra` (`ReactNode`): contenido a la derecha de la barra inferior, junto a `barRight` (p. ej. el QR de Data Fiscal); la barra se muestra también si sólo viene `barExtra`. Sin `barExtra`, el markup de la barra es idéntico al de antes.
